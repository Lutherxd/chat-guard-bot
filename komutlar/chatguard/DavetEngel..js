...
if(!args[0]) {
    ...
    **\`${ayarlar.prefix}davetengel kapat\`**
    ...
    <:luffy:822519742110760960> **\`.davetengel aç\` komutunu kullanarak sunucunuzda \`Davet Link\` kullanımını önleyebilirsiniz yani başka sunucuların davet linki paylaşmasına izin verilmiyicektir.**
    
    <:luffy:822519742110760960> **\`.davetengel kapat\` komutunu kullanarak bu sistemi kapatabilirsiniz ama bu sistemi kapatmak sunucuda davet linklerini engellemiyicektir.**
    
    <a:unlem:822522470925336646> **Davet Engel Sistemi :** ${data.davetEngel ? "**\`✅\`** Açık" : "<:kapali:822759611471233035> Kapalı"}`))}
if(args[0] == 'aç') {
    if(data && data.davetEngel == true) return message.channel.send(embed.setDescription(`<:tik:822557127767621683> Davet engel sistemi zaten **aktif!**`))
...
if(args[0] == 'kapat') {
    if(data && data.davetEngel == false) return message.channel.send(embed.setDescription(`<:altinred:822569045668790303> Davet engel sistemi zaten **deaktif!**`))
